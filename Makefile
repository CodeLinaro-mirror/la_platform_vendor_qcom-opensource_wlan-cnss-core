...
KBUILD_OPTIONS += CONFIG_CNSS_UTILS=y CONFIG_CNSS2=y CONFIG_CNSS2_QMI=y CONFIG_CNSS2_DEBUG=y CONFIG_CNSS_QMI_SVC=y
KBUILD_OPTIONS += CONFIG_DUMP_FW_TO_FILE=y CONFIG_CNSS_QCOM_DEVCD_SUPPORT=y CONFIG_USERSPACE_HELPER_SUPPORT=y
ifeq ($(full_tech), 1)
KBUILD_OPTIONS += CONFIG_QRTR=y CONFIG_QRTR_MHI=y
KBUILD_OPTIONS += CONFIG_QCOM_QMI_HELPERS=y
KBUILD_OPTIONS += CONFIG_MHI_BUS=y CONFIG_MHI_BUS_DEBUG=y
KBUILD_OPTIONS += CONFIG_IPC_LOGGING=y CONFIG_IPC_LOGGING_MODULE=y
else
KBUILD_OPTIONS += CONFIG_QRTR=n CONFIG_QRTR_MHI=n
KBUILD_OPTIONS += CONFIG_QCOM_QMI_HELPERS=n
KBUILD_OPTIONS += CONFIG_MHI_BUS=n CONFIG_MHI_BUS_DEBUG=n
KBUILD_OPTIONS += CONFIG_IPC_LOGGING=y CONFIG_IPC_LOGGING_MODULE=n
endif
KBUILD_OPTIONS += CONFIG_MHI_BUS_MISC=y USE_EXTERNAL_CONFIGS=y CONFIG_CNSS_PLAT_IPC_QMI_SVC=y
KBUILD_OPTIONS += CONFIG_PCIE_QCOM_ECAM=y CONFIG_PINCTRL_MSM_NO_EXT=y CONFIG_QLI=y
#KBUILD_OPTIONS += CONFIG_CNSS2_X86=y                
#KBUILD_OPTIONS += CONFIG_CNSS2_SSR_DRIVER_DUMP=y
ifeq ($(interface_type), pcie)
KBUILD_OPTIONS += CONFIG_CNSS2_PCIE=y
